...
from agent.file_sharing_policy import get_block_message as _get_file_sharing_block_message
...
def execute_tool_calls_concurrent(agent, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
    ...
    for tool_call in tool_calls:
        ...
        if _ts_scope_block is not None:
            ...
            # YAMI Regra 2: File sharing policy check
            # Nenhum arquivo deve ser enviado automaticamente
            elif (fs_block_message := _get_file_sharing_block_message(function_name, function_args)) is not None:
                block_result = json.dumps({"error": fs_block_message}, ensure_ascii=False)
                _emit_terminal_post_tool_call(
                    agent,
                    function_name=function_name,
                    function_args=function_args,
                    ...
                    tool_call_id=getattr(tool_call, "id", "") or "",
                    status="blocked",
                    error_type="yami_file_sharing_policy",
                    error_message=fs_block_message,
                    middleware_trace=list(middleware_trace),
                )
            else:
...
def execute_tool_calls_sequential(agent, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
    ...
    for i, tool_call in enumerate(assistant_message.tool_calls, 1):
        ...
        # YAMI Regra 2: File sharing policy check
        # Nenhum arquivo deve ser enviado automaticamente
        _yami_fs_block_msg: Optional[str] = None
        if _block_msg is None:
            _yami_fs_block_msg = _get_file_sharing_block_message(function_name, function_args)
            if _yami_fs_block_msg is not None:
                _block_msg = _yami_fs_block_msg
                _block_error_type = "yami_file_sharing_policy"
