...
const YAMI_APP_NAME = "Yami";
const YAMI_HOME_DIR =
  process.env.YAMI_HOME ||
  path.join(os.homedir(), ".yami");

process.env.YAMI_HOME = YAMI_HOME_DIR;
const YAMI_CONFIG_PATH =
  process.env.YAMI_CONFIG_PATH || path.join(YAMI_HOME_DIR, "yami.json");
process.env.YAMI_CONFIG_PATH = YAMI_CONFIG_PATH;
process.env.OPENCLAW_HOME = process.env.OPENCLAW_HOME || YAMI_HOME_DIR;
process.env.OPENCLAW_CONFIG_PATH =
  process.env.OPENCLAW_CONFIG_PATH || YAMI_CONFIG_PATH;
process.env.OPENCLAW_STATE_DIR = process.env.OPENCLAW_STATE_DIR || YAMI_HOME_DIR;
process.env.OPENCLAW_BRAND = process.env.OPENCLAW_BRAND || YAMI_APP_NAME;
...
const ensureSupportedNodeVersion = () => {
  ...
  process.stderr.write(
    `yami: Node.js v${MIN_NODE_VERSION}+ is required (current: v${process.versions.node}).\n` +
      "If you use nvm, run:\n" +
...
const runRespawnedChild = (command, args, env) => {
  ...
  child.once("error", (error) => {
    detach();
    process.stderr.write(
    `[yami] Failed to respawn launcher: ${
...
const resolveYamiLearningDir = () => path.join(YAMI_HOME_DIR, "learning-engine");

const initializeLearningEngine = async () => {
  try {
    const learningDir = resolveYamiLearningDir();
    process.env.YAMI_LEARNING_HOME = learningDir;
    const enginePath = path.join(learningDir, "index.mjs");
    if (existsSync(enginePath)) {
      const profilePath = path.join(learningDir, "profile.json");
      if (!existsSync(profilePath)) {
        const engineUrl = url.pathToFileURL(enginePath).href;
        const { getInstance } = await import(engineUrl);
        getInstance({ baseDir: learningDir });
      }
    }
  } catch (err) {
    // Learning engine is optional; do not block startup
  }
};
...
const buildMissingEntryErrorMessage = async () => {
  const lines = ["yami: missing dist/entry.(m)js (build output)."];
  ...
  lines.push(
    "For Yami packaged installs, use the bundled Yami runtime instead of a raw archive URL.",
  );
  lines.push("For releases, use the Yami installer/runtime package.");
  return lines.join("\n");
};
...
const resolveLauncherHomeDir = () => {
  const explicit =
    normalizeLauncherHomeValue(process.env.YAMI_HOME) ??
    normalizeLauncherHomeValue(process.env.OPENCLAW_HOME);
  const rawHome =
...
const resolveLauncherConfigPaths = () => {
  const explicit =
    process.env.YAMI_CONFIG_PATH?.trim() || process.env.OPENCLAW_CONFIG_PATH?.trim();
  if (explicit) {
    return [resolveLauncherUserPath(explicit)];
  }
  const stateOverride =
    process.env.YAMI_HOME?.trim() || process.env.OPENCLAW_STATE_DIR?.trim();
  if (stateOverride) {
    const stateDir = resolveLauncherUserPath(stateOverride);
    return [
      path.join(stateDir, "yami.json"),
  ...
  return [
    path.join(homeDir, ".yami", "yami.json"),
    path.join(homeDir, ".yami", "openclaw.json"),
...
function tryOutputLauncherVersion(argv) {
  try {
    ...
    process.stdout.write(commit ? `Yami ${version} (${commit})\n` : `Yami ${version}\n`);
    return true;
  } catch {
    return false;
  }
